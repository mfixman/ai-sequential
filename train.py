...
def train(data_settings, model_settings, train_settings, logger):
	...
	for epoch in range(epoch_start, train_settings['epochs']):
		...
		val_loss, val_score = validation_loop(model, val_loader, criterion, model_settings)

		print(f'Epoch: {epoch+1:02} | Train Loss: {train_loss:.3f} | Val Loss: {val_loss:.3f} | Val Score: {val_score:.3f}')
		if logger:
			logger.log({'train_loss': train_loss, 'validation_loss': val_loss, 'Rouge2-F1': val_score})
...
@torch.no_grad()
def validation_loop(model, val_loader, criterion, model_settings) -> tuple[FloatTensor, FloatTensor]:
	model.eval()
	epoch_loss = 0
	epoch_score = tensor(0.).to(device)
	dec_out = None # Placeholder
	emb_trg = None # Placeholder
	if model_settings['version'] == '1':
		for i, (src, trg) in enumerate(val_loader):
			...
			torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1)
			epoch_loss += loss.item()
			# epoch_score += torchmetrics.functional.text.rouge.rouge_score(output, trg, 'rouge2')['rouge2']
	elif model_settings['version'] == '2':
		for i, (src, trg, tf_src, tf_trg, idf_src, idf_trg) in enumerate(val_loader):
			...
			loss += l1_lambda*l1_norm
			epoch_loss += loss.item()
			epoch_score += rouge_scores(output, trg)['f1']

	avg_loss = epoch_loss / len(val_loader)
	avg_score = epoch_score / len(val_loader)
