...
def parse_args():
	parser = argparse.ArgumentParser(description = 'Runs the test with one element of the training set.')

	parser.add_argument('--device', choices = ['cuda', 'cpu'], help = 'Device to use')
	parser.add_argument('wandb_tag', type = str, choices = ['seq2seq', 'transformer', 'bertformer'], help = 'Whcih model to run the text set on.')
...
def infer(data_settings, model_settings, inference_settings, device):
	# Dataset
...
def main():
	args = parse_args()
	config, checkpoint = get_run(tag = f'{args.wandb_tag}_artifact')
	...
	infer(data_setting, model_setting, inference_setting, args.device)
